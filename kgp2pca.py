...
df_fam = pd.read_csv(args.fam, delim_whitespace = True, header = None, names = ['FID', 'IID', 'PAT', 'MAT', 'SEX', 'PHE'])
df_pop = pd.read_csv(args.pop, delim_whitespace = True)
...
for group in args.groups.split(','):
  if not group in idx:
    sys.stderr.write('kgp2plot.py: Group ' + group + ' undefined\n')
    continue
  sys.stderr.write('kgp2plot.py: Computing PC for group ' + group + '\n')
  gcta64_args = ["gcta64",
  ...
  p.communicate()
  if p.returncode:
    raise Exception('kgp2plot.py: Problems running gcta64')

  # add principal components information
  eigenval = pd.read_csv(args.out + '.' + group.lower() + '.eigenval', header = None, squeeze = True)
  eigenvec = pd.read_csv(args.out + '.' + group.lower() + '.eigenvec', delim_whitespace = True, header = None, names = ['FID', 'IID'] + ['PC' + str(i + 1) for i in range(args.pca)])
